...
export const scrapeTranscript = (): TranscriptSegment[] | null => {
    ...
    const segmentsContainer = document.querySelector('ytd-transcript-segment-list-renderer div#segments-container');
 ...
 */
export const formatTranscript = (transcriptData: TranscriptSegment[]): string => {
    return transcriptData.map(segment => `${segment.timestamp} - ${segment.text}`).join('\n');
}
