    ...
    try {
      ...
      if (response?.action === ACTION.TRANSCRIPTION_RESULT) {
        ...
        errorMessage = `Error: ${response.payload}`;
         ...
         if (!response) {
            errorMessage = "No response from content script. Try reloading the YouTube page and the extension.";
         } else {
      ...
      if (error.message?.includes("Could not establish connection") || error.message?.includes("Receiving end does not exist")) {
          errorMessage = "Cannot connect to the YouTube page. Please reload the page and try again. If the issue persists, reload the extension.";
      } else {
...
<main>
  ...
  <div class="card">
    ...
    {#if transcription}
      <textarea class="message transcript-area" readonly>{transcription}</textarea>
      <!-- <p class="message">{transcription}</p> -->
    {/if}
